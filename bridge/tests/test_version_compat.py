...
def test_core_compatibility_accepts_declared_range(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(bridge_server.ALLOW_INCOMPATIBLE_ENV, raising=False)
    monkeypatch.setattr(bridge_server, "_installed_core_version", lambda: "0.1.0")

    bridge_server.check_core_compatibility()


def test_core_compatibility_fails_closed_for_minor_skew(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(bridge_server.ALLOW_INCOMPATIBLE_ENV, raising=False)
    monkeypatch.setattr(bridge_server, "_installed_core_version", lambda: "0.2.0")

    with pytest.raises(RuntimeError, match="installed memory-seam 0.2.0 is outside"):
        bridge_server.check_core_compatibility()
...
def test_core_compatibility_override_warns_and_continues(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setenv(bridge_server.ALLOW_INCOMPATIBLE_ENV, "1")
    monkeypatch.setattr(bridge_server, "_installed_core_version", lambda: "0.2.0")
    ...
    assert f"continuing because {bridge_server.ALLOW_INCOMPATIBLE_ENV}=1" in captured.err
