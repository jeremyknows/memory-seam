...
DOC = REPO_ROOT / "docs" / "l6aa01-exact-owner-approved-target-ref-packet.md"
DOCS_INDEX = REPO_ROOT / "docs" / "README.md"
CONTRACT_TEST_INVENTORY = REPO_ROOT / "docs" / "contract-test-inventory.md"

TARGET_REF_PATTERN = re.compile(r"^(descriptor|source-card):l6aa/[a-z0-9]+(?:-[a-z0-9]+)*$")
EXACT_TARGET_REFS = {
...
UNSAFE_TARGET_VALUES = (
    ...
    "source-card:l6aa/other-card",
    "descriptor:l6aa/raw-private-source-text",
    "source-card:l6aa/private-path-source-uri",
...
def test_l6aa01_packet_is_discoverable():
    docs_index = normalized(DOCS_INDEX)
    inventory = normalized(CONTRACT_TEST_INVENTORY)

    assert "l6aa01-exact-owner-approved-target-ref-packet.md" in docs_index
    assert "tests/test_l6aa01_exact_owner_approved_target_ref_packet.py" in inventory
    assert "L6AA.01 exact owner-approved target-ref packet and executable ref fixtures" in inventory
    assert "TARGET_REF_PACKET_ONLY_NO_APPROVAL_NO_EXECUTION" in inventory


def test_l6aa01_records_source_floor_and_preauth_anchors():
    text = normalized(DOC)
    ...
    required_terms = (
        ...
        "L6Z #235 / PR #240 merged",
        "#6 comment `4649391691`",
        "#215 comment `4649391836`",
        "Jeremy voice-message anchor recorded for the bounded L6AA rail",
...
def test_l6aa01_defines_exact_issue_bound_executable_target_refs():
    text = normalized(DOC)

    for field in REQUIRED_BINDING_FIELDS:
        assert field in text
    assert "exactly one issue and max one report-safe source-card read" in text
    assert "#242 may proceed only from a fresh owner comment on #242" in text
    assert target_refs_are_exact_and_report_safe(EXACT_TARGET_REFS)


def test_l6aa01_fixture_rejects_unsafe_raw_private_target_values():
...
def test_l6aa01_non_approval_rejects_stale_mismatched_broadened_copied_expired():
    text = normalized(DOC)

    assert "This packet is not approval and cannot be copied or reused as approval" in text
    assert "Approval recognition for #242 must inspect only a fresh issue-bound owner comment on #242" in text
    assert "exact owner approval phrase required: `ABSENT_FROM_PACKET`" in text
    assert "stale copied L6Z proof refs such as `descriptor:l6z/operator-proof` or `source-card:l6z/operator-proof`: `DENY_BEFORE_READ`" in text
...
def test_l6aa01_report_safe_metadata_shape_and_zero_counters():
    text = normalized(DOC)
...
def test_l6aa01_preserves_all_holds_without_live_read_execution():
    text = normalized(DOC)

    for held in PRESERVED_HOLDS:
        assert held in text
    assert "No live/private read is executed by #241" in text
    assert "#242 must deny before read with guarded counters zero" in text
